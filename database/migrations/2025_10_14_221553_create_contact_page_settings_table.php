...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('contact_page_settings', function (Blueprint $table) {
            ...
            $table->string('title')->default('Свяжитесь с нами');
            $table->text('subtitle')->nullable();
            $table->string('map_latitude')->nullable(); // Широта для карты
            $table->string('map_longitude')->nullable(); // Долгота для карты
            $table->integer('map_zoom')->default(10); // Масштаб карты
            $table->text('map_description')->nullable(); // Описание для метки
