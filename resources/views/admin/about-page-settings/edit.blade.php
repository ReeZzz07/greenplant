...
<head>
    ...
    <title>Редактировать настройки страницы "О компании" - GreenPlant</title>
...
<body>
    <div class="header">
        <div class="header-content">
            ...
            <div>
                <a href="{{ route('admin.about-page-settings.index') }}">← Назад к списку</a>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="page-header">
            <h2>✏️ Редактировать настройки страницы "О компании"</h2>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.about-page-settings.update', $setting->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <select name="overlay_type" id="overlay_type" class="form-control">
                            <option value="none" {{ old('overlay_type', $setting->overlay_type ?? 'none') == 'none' ? 'selected' : '' }}>Без наложения</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="range" name="overlay_opacity" id="overlay_opacity" min="0" max="100" value="{{ old('overlay_opacity', $setting->overlay_opacity ?? 50) }}" oninput="document.getElementById('opacity_value').textContent = this.value">
                        <small>Текущее значение: <strong id="opacity_value">{{ old('overlay_opacity', $setting->overlay_opacity ?? 50) }}</strong>%</small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="background_position" id="background_position" class="form-control">
                            <option value="left top" {{ old('background_position', $setting->background_position ?? 'center center') == 'left top' ? 'selected' : '' }}>Слева сверху</option>
                            ...
                            <option value="center center" {{ old('background_position', $setting->background_position ?? 'center center') == 'center center' ? 'selected' : '' }}>По центру</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="background_size" id="background_size" class="form-control">
                            <option value="cover" {{ old('background_size', $setting->background_size ?? 'cover') == 'cover' ? 'selected' : '' }}>Покрыть всю область</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', $setting->title) }}" placeholder="Например: О компании GreenPlant">
                    </div>

                    <div class="form-group">
                        <label for="subtitle">Подзаголовок (опционально)</label>
                        <textarea name="subtitle" id="subtitle" placeholder="Например: Мы специализируемся на выращивании и продаже различных сортов туи">{{ old('subtitle', $setting->subtitle) }}</textarea>
                    </div>

                    <h3>📄 Контент страницы</h3>

                    <div class="form-group">
                        <label for="about_image">Изображение "О компании"</label>
                        @if($setting->about_image)
                            <div class="current-image">
                                <strong>Текущее изображение:</strong><br>
                                <img src="{{ asset('storage/' . $setting->about_image) }}" alt="Current" class="image-preview">
                            </div>
                        @endif
                        <input type="file" name="about_image" id="about_image" accept="image/*">
                        <small>Рекомендуемый размер: 600x400px. Форматы: JPG, PNG, GIF (макс. 2 МБ)</small>
                    </div>

                    <div class="form-group">
                        <label for="welcome_title">Заголовок приветствия</label>
                        <input type="text" name="welcome_title" id="welcome_title" value="{{ old('welcome_title', $setting->welcome_title) }}" placeholder="Например: Добро пожаловать в GreenPlant">
                    </div>

                    <div class="form-group">
                        <label for="welcome_text">Текст приветствия</label>
                        <textarea name="welcome_text" id="welcome_text" placeholder="Например: GreenPlant - это питомник растений...">{{ old('welcome_text', $setting->welcome_text) }}</textarea>
                    </div>

                    <div class="form-group">
                        <label for="main_content">Дополнительный контент</label>
                        <textarea name="main_content" id="main_content" placeholder="Дополнительная информация о компании...">{{ old('main_content', $setting->main_content) }}</textarea>
                    </div>
                    ...
                    <div class="btn-group">
                        ...
                        <a href="{{ route('admin.about-page-settings.index') }}" class="btn btn-secondary">❌ Отмена</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
        ...
        function updatePreview() {
            ...
            if (fileInput.files && fileInput.files[0]) {
                ...
                const currentImage = document.getElementById('current-bg-image');
                if (currentImage) {
