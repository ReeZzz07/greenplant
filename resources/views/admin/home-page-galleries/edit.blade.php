...
<head>
    ...
    <title>Редактировать изображение галереи - GreenPlant</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .current-image img { max-width: 300px; height: auto; border-radius: 10px; }
    </style>
</head>
<body>
    <div class="header">
    ...
    <div class="container">
        <div class="page-header">
            <h2>Редактировать изображение галереи</h2>
        </div>

        <div class="card">
            <form action="{{ route('admin.home-page-galleries.update', $gallery->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <input type="url" class="@error('link') is-invalid @enderror" id="link" name="link" value="{{ old('link', $gallery->link) }}" placeholder="https://www.instagram.com/p/...">
                    @error('link')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" class="@error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $gallery->order) }}">
                    </div>
                    <div class="form-group">
                        <label for="is_active">Статус</label>
                        <select class="@error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                            <option value="1" {{ old('is_active', $gallery->is_active) == 1 ? 'selected' : '' }}>Активен</option>
                            <option value="0" {{ old('is_active', $gallery->is_active) == 0 ? 'selected' : '' }}>Неактивен</option>
                        </select>
                    </div>
                </div>
