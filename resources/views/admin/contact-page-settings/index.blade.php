...
<head>
    ...
    <title>Настройки страницы "Контакты" - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>📞 Настройки страницы "Контакты"</h2>
            <a href="{{ route('admin.contact-page-settings.create') }}" class="btn btn-primary">+ Создать настройки</a>
        </div>
        ...
        <div class="card">
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($setting->map_embed_code)
                                        <span style="color: #28a745; font-weight: 600;">✓ Код карты</span>
                                    @elseif($setting->map_latitude && $setting->map_longitude)
                                        <span style="color: #ffc107;">⚠ API (устаревшее)</span>
                                    @else
                                        <span style="color: #999;">—</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('admin.contact-page-settings.edit', $setting->id) }}" class="btn btn-sm btn-primary">✏️ Редактировать</a>
                                        <form action="{{ route('admin.contact-page-settings.destroy', $setting->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Вы уверены?');">
                                            @csrf
                                            @method('DELETE')
            ...
            @else
                <div class="empty-state">
                    ...
                    <h3>Настройки страницы "Контакты" не найдены</h3>
                    <p style="margin-top: 10px;">Создайте первую настройку</p>
                    <a href="{{ route('admin.contact-page-settings.create') }}" class="btn btn-primary" style="margin-top: 20px;">Создать настройки</a>
                </div>
            @endif
        </div>
    </div>
