...
<head>
    ...
    <title>Создать настройки страницы "Контакты" - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>➕ Создать настройки страницы "Контакты"</h2>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.contact-page-settings.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <div id="preview-box" style="width: 100%; height: 200px; border: 2px solid #e9ecef; border-radius: 10px; position: relative; overflow: hidden; background: #f8f9fa; display: flex; align-items: center; justify-content: center;">
                            ...
                            <div id="preview-content" style="position: relative; z-index: 1; text-align: center; color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">
                                <h3 style="margin: 0; font-size: 24px;">Свяжитесь с нами</h3>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" value="{{ old('title', 'Свяжитесь с нами') }}" placeholder="Например: Свяжитесь с нами">
                    </div>

                    <div class="form-group">
                        <label for="subtitle">Подзаголовок (опционально)</label>
                        <textarea name="subtitle" id="subtitle" placeholder="Например: Мы всегда рады ответить на ваши вопросы">{{ old('subtitle') }}</textarea>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="map_embed_code" id="map_embed_code" rows="6" placeholder='Например: <iframe src="https://yandex.ru/map-widget/..." width="100%" height="400"></iframe>'>{{ old('map_embed_code') }}</textarea>
                    ...
                    <details style="margin-top: 15px; padding: 15px; background: #f8f9fa; border-radius: 10px;">
                        ...
                        <div class="form-row" style="margin-top: 15px;">
                            <div class="form-group">
                                ...
                                <input type="text" name="map_latitude" id="map_latitude" value="{{ old('map_latitude', '55.751574') }}" placeholder="Например: 55.751574">
                                <small>Координата широты для центра карты</small>
                            </div>

                            <div class="form-group">
                                <label for="map_longitude">Долгота (Longitude)</label>
                                <input type="text" name="map_longitude" id="map_longitude" value="{{ old('map_longitude', '37.573856') }}" placeholder="Например: 37.573856">
                        ...
                        <div class="form-group">
                            ...
                            <input type="range" name="map_zoom" id="map_zoom" min="1" max="20" value="{{ old('map_zoom', 10) }}" oninput="document.getElementById('zoom_value').textContent = this.value">
                            <small>Текущее значение: <strong id="zoom_value">10</strong></small>
                        </div>

                        <div class="form-group">
                            <label for="map_description">Описание для метки на карте</label>
                            <textarea name="map_description" id="map_description" rows="2" placeholder="Например: Наш питомник GreenPlant">{{ old('map_description') }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="contact_section_title" id="contact_section_title" value="{{ old('contact_section_title', 'Свяжитесь с нами') }}" placeholder="Например: Свяжитесь с нами">
                        <small>Заголовок секции с контактной информацией</small>
                    </div>

                    <div class="form-group">
                        <label for="contact_section_content">Контент секции</label>
                        <textarea name="contact_section_content" id="contact_section_content" rows="8" placeholder="Например: Мы всегда рады ответить на ваши вопросы...">{{ old('contact_section_content') }}</textarea>
                        <small>Текст, который будет отображаться в секции с контактами. Можно использовать HTML-теги для форматирования.</small>
                    </div>

                    <h3>📝 Текст согласия</h3>

                    <div class="form-group">
                        <label for="consent_text">Текст согласия под капчей</label>
                        <textarea name="consent_text" id="consent_text" rows="3" placeholder='Нажимая кнопку "Отправить сообщение", Вы соглашаетесь с условиями "Политики конфиденциальности" и "Пользовательским соглашением"'>{{ old('consent_text') }}</textarea>
                        <small>Текст, который отображается после капчи перед кнопкой отправки. Можно использовать HTML-теги для форматирования ссылок. Если не заполнено, будет использован текст по умолчанию.</small>
                    </div>
