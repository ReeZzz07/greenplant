...
<head>
    ...
    <title>Редактировать преимущество - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        ...
        <div class="card">
            <form action="{{ route('admin.home-page-features.update', $feature->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="@error('icon_type') is-invalid @enderror" id="icon_type" name="icon_type" required onchange="toggleIconFields()">
                        <option value="flaticon" {{ old('icon_type', $feature->icon_type) == 'flaticon' ? 'selected' : '' }}>Flaticon (из набора)</option>
                        <option value="image" {{ old('icon_type', $feature->icon_type) == 'image' ? 'selected' : '' }}>Изображение (PNG/SVG)</option>
                        <option value="custom" {{ old('icon_type', $feature->icon_type) == 'custom' ? 'selected' : '' }}>Кастомный код</option>
                    </select>
                    @error('icon_type')
                ...
                <div class="form-group" id="flaticon-field" style="display: none;">
                    ...
                    <select class="@error('icon') is-invalid @enderror" id="icon" name="icon">
                        ...
                        <option value="flaticon-bag" {{ old('icon', $feature->icon) == 'flaticon-bag' ? 'selected' : '' }}>🛍️ Корзина (flaticon-bag)</option>
                        <option value="flaticon-customer-service" {{ old('icon', $feature->icon) == 'flaticon-customer-service' ? 'selected' : '' }}>📞 Поддержка (flaticon-customer-service)</option>
                        <option value="flaticon-payment-security" {{ old('icon', $feature->icon) == 'flaticon-payment-security' ? 'selected' : '' }}>🔒 Безопасность (flaticon-payment-security)</option>
                        <option value="flaticon-heart-box" {{ old('icon', $feature->icon) == 'flaticon-heart-box' ? 'selected' : '' }}>❤️ Сердце (flaticon-heart-box)</option>
                    </select>
                    @error('icon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group" id="image-field" style="display: none;">
                    ...
                    @if($feature->icon_image)
                        <div style="margin-bottom: 10px;">
                            <img src="{{ asset('storage/' . $feature->icon_image) }}" alt="" style="max-width: 100px; height: auto; border-radius: 8px;">
                ...
                <div class="form-group" id="custom-field" style="display: none;">
                    ...
                    <input type="text" class="@error('icon') is-invalid @enderror" id="icon_custom" name="icon" value="{{ old('icon', $feature->icon) }}" placeholder="Например: &lt;i class='fas fa-star'&gt;&lt;/i&gt;">
                    @error('icon')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select class="@error('icon_size') is-invalid @enderror" id="icon_size" name="icon_size">
                            <option value="32px" {{ old('icon_size', $feature->icon_size ?? '48px') == '32px' ? 'selected' : '' }}>Маленький (32px)</option>
                            <option value="48px" {{ old('icon_size', $feature->icon_size ?? '48px') == '48px' ? 'selected' : '' }}>Средний (48px)</option>
                            <option value="64px" {{ old('icon_size', $feature->icon_size ?? '48px') == '64px' ? 'selected' : '' }}>Большой (64px)</option>
                            <option value="80px" {{ old('icon_size', $feature->icon_size ?? '48px') == '80px' ? 'selected' : '' }}>Очень большой (80px)</option>
                            <option value="custom" {{ old('icon_size', $feature->icon_size ?? '48px') == 'custom' ? 'selected' : '' }}>Свой размер</option>
                        </select>
                        @error('icon_size')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="form-group" id="custom-size-field" style="display: none;">
                        ...
                        <input type="text" class="@error('icon_size') is-invalid @enderror" id="icon_size_custom" name="icon_size" value="{{ old('icon_size', $feature->icon_size) }}" placeholder="56px">
                        @error('icon_size')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <input type="color" id="icon_color_picker" value="{{ old('icon_color', $feature->icon_color ?? '#82ae46') }}" style="width: 60px; height: 40px; border: none; border-radius: 8px; cursor: pointer;">
                            <input type="text" class="@error('icon_color') is-invalid @enderror" id="icon_color" name="icon_color" value="{{ old('icon_color', $feature->icon_color ?? '#82ae46') }}" placeholder="#82ae46" style="flex: 1;">
                        </div>
                        @error('icon_color')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="@error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title', $feature->title) }}" required>
                    @error('title')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="@error('description') is-invalid @enderror" id="description" name="description" rows="3" required>{{ old('description', $feature->description) }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" class="@error('order') is-invalid @enderror" id="order" name="order" value="{{ old('order', $feature->order) }}">
                    </div>
                    <div class="form-group">
                        <label for="is_active">Статус</label>
                        <select class="@error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                            <option value="1" {{ old('is_active', $feature->is_active) == 1 ? 'selected' : '' }}>Активен</option>
                            <option value="0" {{ old('is_active', $feature->is_active) == 0 ? 'selected' : '' }}>Неактивен</option>
                        </select>
                    </div>
                </div>
