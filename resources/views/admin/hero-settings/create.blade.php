...
<head>
    ...
    <title>Создать настройки Hero-секции - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>Создать настройки Hero-секции</h2>
        </div>

        <div class="card">
            <form action="{{ route('admin.hero-settings.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    @enderror
                    <div class="help-text">Загрузите изображение для фона hero-секции (рекомендуемый размер: 1920x1080px)</div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <select class="@error('background_position') is-invalid @enderror" id="background_position" name="background_position" onchange="updatePreview()">
                            ...
                            <option value="top center" {{ old('background_position') == 'top center' ? 'selected' : '' }}>Сверху по центру</option>
                            <option value="bottom center" {{ old('background_position') == 'bottom center' ? 'selected' : '' }}>Снизу по центру</option>
                            ...
                            <option value="top left" {{ old('background_position') == 'top left' ? 'selected' : '' }}>Сверху слева</option>
                            <option value="top right" {{ old('background_position') == 'top right' ? 'selected' : '' }}>Сверху справа</option>
                            <option value="bottom left" {{ old('background_position') == 'bottom left' ? 'selected' : '' }}>Снизу слева</option>
                            <option value="bottom right" {{ old('background_position') == 'bottom right' ? 'selected' : '' }}>Снизу справа</option>
                            <option value="custom" {{ old('background_position') == 'custom' ? 'selected' : '' }}>Своя позиция</option>
                        </select>
                    ...
                    <div class="form-group" id="custom-position-field" style="display: none;">
                        ...
                        <input type="text" class="@error('background_position') is-invalid @enderror" id="background_position_custom" name="background_position" value="{{ old('background_position') }}" placeholder="30% 70%">
                        @error('background_position')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="form-group">
                        ...
                        <select class="@error('background_size') is-invalid @enderror" id="background_size" name="background_size" onchange="updatePreview()">
                            <option value="cover" {{ old('background_size', 'cover') == 'cover' ? 'selected' : '' }}>Cover (заполнить)</option>
                            <option value="contain" {{ old('background_size') == 'contain' ? 'selected' : '' }}>Contain (вписать)</option>
                            <option value="auto" {{ old('background_size') == 'auto' ? 'selected' : '' }}>Auto (оригинал)</option>
                            <option value="100% 100%" {{ old('background_size') == '100% 100%' ? 'selected' : '' }}>Растянуть (100%)</option>
                            <option value="custom" {{ old('background_size') == 'custom' ? 'selected' : '' }}>Свой размер</option>
                        </select>
                    ...
                    <div class="form-group" id="custom-size-field" style="display: none;">
                        ...
                        <input type="text" class="@error('background_size') is-invalid @enderror" id="background_size_custom" name="background_size" value="{{ old('background_size') }}" placeholder="150% 150%">
                        @error('background_size')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <div style="display: flex; gap: 10px; align-items: center;">
                            <input type="color" id="background_color_picker" value="#82ae46" style="width: 60px; height: 40px; border: none; border-radius: 8px; cursor: pointer;">
                            <input type="text" class="@error('background_color') is-invalid @enderror" id="background_color" name="background_color" value="{{ old('background_color', '#82ae46') }}" placeholder="#82ae46" style="flex: 1;">
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="@error('overlay_type') is-invalid @enderror" id="overlay_type" name="overlay_type" required onchange="updatePreview()">
                        <option value="darken" {{ old('overlay_type', 'darken') == 'darken' ? 'selected' : '' }}>Затемнение</option>
                        <option value="lighten" {{ old('overlay_type') == 'lighten' ? 'selected' : '' }}>Осветление</option>
                        <option value="none" {{ old('overlay_type') == 'none' ? 'selected' : '' }}>Без наложения</option>
                    </select>
                    @error('overlay_type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="form-group">
                    <label for="overlay_opacity">Прозрачность наложения: <span class="range-value" id="opacity-value">50</span>%</label>
                    <input type="range" class="range-input @error('overlay_opacity') is-invalid @enderror" id="overlay_opacity" name="overlay_opacity" min="0" max="100" value="{{ old('overlay_opacity', 50) }}" oninput="updateOpacity(this.value)">
