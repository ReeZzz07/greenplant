...
<head>
    ...
    <title>Настройки страницы "Оптовым покупателям" - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>💰 Настройки страницы "Оптовым покупателям"</h2>
            <a href="{{ route('admin.wholesale-settings.create') }}" class="btn btn-primary">+ Создать настройки</a>
        </div>
        ...
        <div class="card">
                    ...
                    <tbody>
                        @foreach($settings as $setting)
                            <tr>
                                ...
                                <td>{{ $setting->seedling_price ?? $setting->purchase_price ?? '—' }} ₽</td>
                                <td>
                                ...
                                <td>
                                    <div class="actions">
                                        <a href="{{ route('admin.wholesale-settings.edit', $setting->id) }}" class="btn btn-sm btn-primary">✏️ Редактировать</a>
                                        <form action="{{ route('admin.wholesale-settings.destroy', $setting->id) }}" method="POST" style="display: inline-block;" onsubmit="return confirm('Вы уверены?');">
                                            @csrf
                                            @method('DELETE')
            ...
            @else
                <div class="empty-state">
                    ...
                    <h3>Настройки страницы "Оптовым покупателям" не найдены</h3>
                    <p style="margin-top: 10px;">Создайте первую настройку</p>
                    <a href="{{ route('admin.wholesale-settings.create') }}" class="btn btn-primary" style="margin-top: 20px;">Создать настройки</a>
                </div>
            @endif
        </div>
    </div>
