...
<head>
    ...
    <title>Редактировать настройки страницы "Оптовым покупателям" - GreenPlant</title>
    <style>
...
<body>
    ...
    <div class="container">
        <div class="page-header">
            <h2>✏️ Редактировать настройки страницы "Оптовым покупателям"</h2>
        </div>
        ...
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.wholesale-settings.update', $setting->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group">
                        ...
                        <div id="preview-box" style="width: 100%; height: 200px; border: 2px solid #e9ecef; border-radius: 10px; position: relative; overflow: hidden; background: #f8f9fa; display: flex; align-items: center; justify-content: center;">
                            <div id="preview-background" style="position: absolute; top: 0; left: 0; right: 0; bottom: 0; background-size: {{ $setting->background_size ?? 'cover' }}; background-position: {{ $setting->background_position ?? 'center center' }}; background-repeat: no-repeat; background-image: url('{{ $setting->background_image ? asset('storage/' . $setting->background_image) : '' }}');"></div>
                            ...
                            <div id="preview-content" style="position: relative; z-index: 1; text-align: center; color: white; text-shadow: 0 2px 4px rgba(0,0,0,0.3);">
                                ...
                                @if($setting->subtitle)
                                    <p style="margin: 5px 0 0 0; font-size: 14px; opacity: 0.9;">{{ Str::limit($setting->subtitle, 50) }}</p>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="range" name="overlay_opacity" id="overlay_opacity" min="0" max="100" value="{{ $setting->overlay_opacity ?? 50 }}" oninput="document.getElementById('opacity_value').textContent = this.value; updatePreview();">
                        <small>Текущее значение: <strong id="opacity_value">{{ $setting->overlay_opacity ?? 50 }}</strong>%</small>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="background_position" id="background_position" onchange="updatePreview()">
                            <option value="left top" {{ ($setting->background_position ?? 'center center') == 'left top' ? 'selected' : '' }}>Слева сверху</option>
                            ...
                            <option value="center center" {{ ($setting->background_position ?? 'center center') == 'center center' ? 'selected' : '' }}>По центру</option>
                    ...
                    <div class="form-group">
                        ...
                        <select name="background_size" id="background_size" onchange="updatePreview()">
                            <option value="cover" {{ ($setting->background_size ?? 'cover') == 'cover' ? 'selected' : '' }}>Покрыть всю область</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="background_color" id="background_color" value="{{ $setting->background_color ?? '#82ae46' }}" placeholder="#82ae46">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="title" id="title" value="{{ $setting->title }}" placeholder="Например: Оптовым покупателям">
                    </div>

                    <div class="form-group">
                        <label for="subtitle">Подзаголовок (опционально)</label>
                        <textarea name="subtitle" id="subtitle" rows="3" placeholder="Например: Поставляйте туи «Смарагд» с прибылью...">{{ $setting->subtitle }}</textarea>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="seedling_price" id="seedling_price" step="0.01" min="0" value="{{ $setting->seedling_price ?? $setting->purchase_price ?? 300 }}">
                        </div>
                        <div class="form-group">
                            <label for="mature_tree_price">Цена продажи взрослого дерева (₽)</label>
                            <input type="number" name="mature_tree_price" id="mature_tree_price" step="0.01" min="0" value="{{ $setting->mature_tree_price ?? 3800 }}">
                        </div>
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" name="planting_distance" id="planting_distance" step="0.1" min="0.1" value="{{ $setting->planting_distance ?? 0.8 }}">
                        </div>
                        <div class="form-group">
                            <label for="maturity_years">Количество лет до продажи</label>
                            <input type="number" name="maturity_years" id="maturity_years" min="1" value="{{ $setting->maturity_years ?? 3 }}">
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="calculator_description">Описание под калькулятором</label>
                        <textarea name="calculator_description" id="calculator_description" rows="3" placeholder="Например: Через 3 года вы сможете продать взрослые деревья...">{{ $setting->calculator_description }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="advantages" id="advantages" class="json-field" rows="10">{{ $setting->advantages ? json_encode($setting->advantages, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) : '' }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="how_it_works" id="how_it_works" class="json-field" rows="10">{{ $setting->how_it_works ? json_encode($setting->how_it_works, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) : '' }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="how_it_works_text" id="how_it_works_text" rows="3" placeholder="Например: «Мы не просто поставляем растения — мы поставляем вашу прибыль»">{{ old('how_it_works_text', $setting->how_it_works_text) }}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="testimonials" id="testimonials" class="json-field" rows="10">{{ $setting->testimonials ? json_encode($setting->testimonials, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT) : '' }}</textarea>
                ...
                <form id="delete-form" action="{{ route('admin.wholesale-settings.destroy', $setting->id) }}" method="POST" style="display: none;">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
