...
<head>
    ...
    <title>Редактировать статью - GreenPlant</title>
...
<body>
    ...
    <div class="container">
        <div class="card">
            <h2>Редактировать: {{ $blog->title }}</h2>
            ...
            <form action="{{ route('admin.blog.update', $blog) }}" method="POST" enctype="multipart/form-data" id="blogForm">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <textarea id="excerpt" name="excerpt">{{ old('excerpt', $blog->excerpt) }}</textarea>
                    @error('excerpt')<div class="error">{{ $message }}</div>@enderror
                </div>

                <div class="form-group">
                    <label for="content">Содержание статьи *</label>
                    <textarea id="content" name="content" style="min-height: 300px;">{{ old('content', $blog->content) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    @if($blog->image)
                        <img src="{{ asset('storage/' . $blog->image) }}" alt="{{ $blog->title }}" class="current-image">
                    @endif
